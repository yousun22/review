...
function updateValveState(newState, phonenum) {
    ...
    [newState, phonenum, phonenum], (err, result) => {
        if (err) {
            ...
            globalToggleState = newState;
            console.log(`밸브 상태가 바뀌었습니다: ${newState}`);
        }
    });
}
...
var server = net.createServer(function(socket) {
    ...
    socket.on('data', function(data) {
        ...
        if (receivedData === 'OCK' || receivedData === 'FCK') {
            ...
            updateValveState(receivedData === 'OCK' ? 'ON' : 'OFF', specificPhonenum);
            return;
        }
        ...
        if (receivedData === 'OND' || receivedData === 'FND') {
            ...
            if (timeElapsed <= 29000) {
                ...
                const toggleStateString = globalToggleState === 'ON' ? 'AD' : 'HZ';
                
                // 1초 후에 명령을 재전송
                setTimeout(() => {
                    resendToggleCommand(clients[clientKey], toggleStateString, clientKey, specificPhonenum);
                }, 1000);
            }
            return;
        }
        ...
        client.socket.write(toggleStateString, 'utf8', (err) => {
            ...
            client.socket.once('data', (data) => {
                ...
                if (receivedData === 'FCK') {
                    ...
                    updateValveState('OFF', specificPhonenum);
                    ...
                    updateValveState('ON', specificPhonenum);
