...
function fetchLatestToggleStates() {
    pool.query('SELECT phonenum, valve_of, water_level_setting FROM (SELECT phonenum, valve_of, water_level_setting, ROW_NUMBER() OVER (PARTITION BY phonenum ORDER BY created_at DESC) as rnum FROM waterm) temp WHERE rnum = 1', 
    (err, results) => {
        if (err) {
            ...
            results.forEach(result => {
                ...
                if (waterLevel) {
                    globalSettings[phonenum] = { ...globalSettings[phonenum], waterLevel }; // 이전 waterLevel 반영
                } else {
                    globalSettings[phonenum] = { ...globalSettings[phonenum], waterLevel: 0 }; // 기본값
                }
            });
...
app.post('/save_water_level', (req, res) => {
    ...
    const waterLevel = req.body.waterLevel; // 클라이언트에서 전달된 waterLevel 값
    const mode = req.body.mode;

    const modeValue = mode === 'manual' ? 'M' : mode === 'auto' ? 'A' : 'S';
    ...
    pool.query(
        ...
        [phonenum, globalToggleStates[phonenum], modeValue, waterLevel, createdAt], // 올바른 waterLevel 값 확인
        (err, result) => {
            if (err) {
...
// waterdata와 water_level_setting을 비교하는 함수
function compareWaterLevelAndData(phonenum, waterdata) {
    // 비교 기준 변수 추가
    const water_level_setting_limit1 = 200;

    // 가장 최근의 water_level_setting 가져오기
    pool.query(
        'SELECT water_level_setting FROM waterm WHERE phonenum = ? ORDER BY created_at DESC LIMIT 1',
        [phonenum],
        (err, results) => {
            if (err) {
                console.error('Error retrieving water level setting for comparison:', err);
                return;
            }
            ...
            if (results.length > 0) {
                ...
                // waterdata가 null 또는 0이 아닌 경우에만 비교
                if (waterdata === null || waterdata === 0) {
                    console.log('waterdata가 유효하지 않습니다. 비교를 건너뜁니다.');
                    return;
                }

                console.log(`최근 waterdata: ${waterdata}, water_level_setting: ${waterLevelSetting}, limit1: ${water_level_setting_limit1}`);
                
                if (waterdata > waterLevelSetting) {
                    console.log('Waterdata가 설정한 수위를 초과합니다.');
                } else {
                    console.log('Waterdata가 설정한 수위 이하입니다.');
                }

                // 추가로 limit1과 waterdata 비교
                if (waterdata > water_level_setting_limit1) {
                    console.log(`Waterdata가 limit1(${water_level_setting_limit1})을 초과했습니다.`);
                } else {
                    console.log(`Waterdata가 limit1(${water_level_setting_limit1}) 이하입니다.`);
                }
            } else {
