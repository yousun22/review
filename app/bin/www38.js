...
function compareWaterLevelAndData(phonenum, waterdata) {
   ...
   // 가장 최근의 water_level_setting과 mode 가져오기
   pool.query(
       'SELECT water_level_setting, mode FROM waterm WHERE phonenum = ? ORDER BY created_at DESC LIMIT 1',
       [phonenum],
       (err, results) => {
           if (err) {
               console.error('Error retrieving water level setting and mode for comparison:', err);
               return;
           }
           ...
           if (results.length > 0) {
               ...
               console.log(`최근 waterdata: ${waterdata}, water_level_setting: ${waterLevelSetting}, limit1: ${water_level_setting_limit1}, mode: ${mode}`);

               // 모드가 'auto'(A)인 경우에만 명령을 보낼 수 있음
               if (mode !== 'auto') {
                   console.log('모드가 auto가 아니므로 명령을 보내지 않습니다.');
                   return;
               }

               // 수위가 설정한 수위를 초과하는 경우 닫힘 명령(HZHZHZHZHZ)을 한 번 전송
               if (waterdata > waterLevelSetting) {
                   console.log('Waterdata가 설정한 수위를 초과했습니다. 닫힘 명령을 보냅니다.');
                   sendCommand(phonenum, "HZHZHZHZHZ"); // 닫힘 명령 전송
               }
               // 수위가 limit1 이하인 경우 열림 명령(ADADADADAD)을 한 번 전송
               else if (waterdata <= water_level_setting_limit1) {
                   console.log(`Waterdata가 limit1(${water_level_setting_limit1}) 이하입니다. 열림 명령을 보냅니다.`);
                   sendCommand(phonenum, "ADADADADAD"); // 열림 명령 전송
               } else {
