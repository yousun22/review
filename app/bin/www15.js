        ...
        client.socket.write(toggleStateString, 'utf8', (err) => {
            ...
            client.socket.once('data', (data) => {
                if (data.toString().trim() === 'ACK') {
                    console.log('ACK received from client on resend:', clientKey);

                    connection.query('UPDATE waterm SET valve_of = ? WHERE phonenum = ? AND created_at = (SELECT * FROM (SELECT MAX(created_at) FROM waterm WHERE phonenum = ?) AS temp)', [globalToggleState, phonenum, phonenum], (err, result) => {
                        if (err) {
                            console.error('Error updating valve_of in MySQL on resend:', err);
                        } else {
                            console.log('Valve state updated in MySQL on resend.');
                        }
                    });
                } else {
                    console.log('Non-ACK message received on resend:', data.toString());
                }
            });
        });
    } else {
...
var server = net.createServer(function(socket) {
    ...
    socket.on('data', function(data) {
        ...
        if (receivedData.startsWith('PD#')) {
            ...
            const actuationTime = parseInt(dataParts[1]);
            ...
            if (phonenum && actuationTime < 30000) {
                console.log('Actuation time is less than 20 seconds. Resending toggle command.');
                const toggleStateString = globalToggleState === 'ON' ? "AB" : "CD";
                resendToggleCommand(clients[clientKey], toggleStateString, clientKey, phonenum);
            }
        } else {
            const dataParts = receivedData.split('!');
