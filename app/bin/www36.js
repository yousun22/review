...
function fetchLatestToggleStates() {
    pool.query('SELECT phonenum, valve_of FROM (SELECT phonenum, valve_of, ROW_NUMBER() OVER (PARTITION BY phonenum ORDER BY created_at DESC) as rnum FROM waterm) temp WHERE rnum = 1', 
    (err, results) => {
        if (err) {
...
function compareWaterLevelAndData(phonenum, waterdata) {
    // 비교 기준 변수 추가
    const water_level_setting_limit1 = 300;
